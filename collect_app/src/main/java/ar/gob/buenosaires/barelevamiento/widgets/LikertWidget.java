...
import static ar.gob.buenosaires.barelevamiento.analytics.AnalyticsEvents.LIKERT;
import static ar.gob.buenosaires.barelevamiento.utilities.ViewUtils.dpFromPx;
...
public class LikertWidget extends ItemsWidget {
    ...
    public LikertWidget(Context context, QuestionDetails questionDetails) {
        ...
        analytics.logEvent(AnalyticsEvents.LIKERT, questionDetails.getFormAnalyticsID());
    }
    ...
    public void addLine(boolean left, boolean right, RadioButton button, RelativeLayout buttonView) {
        ...
        if (left) {
            if (isRTL()) {
                rightLineView.setVisibility(INVISIBLE);
            } else {
                leftLineView.setVisibility(INVISIBLE);
            }
        }
