...
import ar.gob.buenosaires.barelevamiento.provider.FormsProvider;
...
import ar.gob.buenosaires.barelevamiento.analytics.Analytics;
import ar.gob.buenosaires.barelevamiento.analytics.AnalyticsEvents;
import ar.gob.buenosaires.barelevamiento.dao.FormsDao;
import ar.gob.buenosaires.barelevamiento.dao.InstancesDao;
...
import static ar.gob.buenosaires.barelevamiento.analytics.AnalyticsEvents.SCOPED_STORAGE_MIGRATION;
import static ar.gob.buenosaires.barelevamiento.preferences.GeneralKeys.KEY_REFERENCE_LAYER;
import static ar.gob.buenosaires.barelevamiento.provider.FormsProviderAPI.FormsColumns.FORM_FILE_PATH;
import static ar.gob.buenosaires.barelevamiento.provider.FormsProviderAPI.FormsColumns.FORM_MEDIA_PATH;
import static ar.gob.buenosaires.barelevamiento.provider.FormsProviderAPI.FormsColumns.JRCACHE_FILE_PATH;
...
public class StorageMigrator {
    ...
    void performStorageMigration() {
        ...
        analytics.logEvent(AnalyticsEvents.SCOPED_STORAGE_MIGRATION, result.toString());
    }
    ...
    private void migrateInstancesDatabase() {
        ...
        if (cursor != null && cursor.moveToFirst()) {
            do {
                ...
                values.put(InstanceProviderAPI.InstanceColumns.INSTANCE_FILE_PATH, getRelativeInstanceDbPath(cursor.getString(instanceFilePathColumnIndex)));
                values.put(InstanceProviderAPI.InstanceColumns.LAST_STATUS_CHANGE_DATE, cursor.getLong(lastStatusChangeDateColumnIndex));
    ...
    private void migrateFormsDatabase() {
        ...
        if (cursor != null && cursor.moveToFirst()) {
            do {
                ...
                ContentValues values = new ContentValues();
                values.put(FormsProviderAPI.FormsColumns.FORM_FILE_PATH, getRelativeFormDbPath(cursor.getString(formFilePathColumnIndex)));
                values.put(FormsProviderAPI.FormsColumns.FORM_MEDIA_PATH, getRelativeFormDbPath(cursor.getString(formMediaPathColumnIndex)));
                values.put(FormsProviderAPI.FormsColumns.JRCACHE_FILE_PATH, getRelativeCacheDbPath(cursor.getString(jrCacheFilePathColumnIndex)));
    ...
    private void migrateMapLayerPath() {
        try {
            String layerPath = (String) generalSharedPreferences.get(GeneralKeys.KEY_REFERENCE_LAYER);
            if (layerPath != null && !layerPath.isEmpty()) {
                generalSharedPreferences.save(GeneralKeys.KEY_REFERENCE_LAYER, storagePathProvider.getRelativeMapLayerPath(layerPath));
            }
        } catch (Exception | Error e) {
            generalSharedPreferences.reset(GeneralKeys.KEY_REFERENCE_LAYER);
        }
    }
}